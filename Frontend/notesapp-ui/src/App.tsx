...
function NotesApp() {
  ...
  // Warm up the API on first render
    ...
    <AppLayout>
      ...
      <div className="flex items-center justify-end mb-4">
        <div className="flex gap-3 items-center">
          <Input
            ...
            className="w-56 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600 placeholder:text-gray-400 dark:placeholder:text-gray-500"
          />
          <select
            value={pageSize}
            onChange={(e) => { setPage(1); setPageSize(Number(e.target.value)); }}
            className="border px-2 py-1 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 border-gray-300 dark:border-gray-600"
          >
      ...
      {loading ? (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
          {Array.from({ length: pageSize }).map((_, i) => <NoteCardSkeleton key={i} />)}
        </div>
      ) : filteredNotes.length === 0 ? (
        <div className="text-center py-20 space-y-4">
          <div className="text-6xl">📝</div>
          <h2 className="text-xl font-semibold">No notes found</h2>
          <p className="text-gray-500 dark:text-gray-400">Create your first note or try a different search.</p>
          <Button onClick={() => setModalOpen(true)}>➕ Create your first note</Button>
        </div>
      ) : (
        <NotesList
          notes={filteredNotes}
          ...
          onDelete={async (id) => { await deleteNote(id); setPage(1); }}
